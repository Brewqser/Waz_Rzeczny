...
namespace Brewque
{
	Game_State::Game_State(GameDataRef data) : _data(data)
	{
	...
	void Game_State::Update(float dt)
	{
		if (_gamestate == GameStates::Playing)
		{
			if (_clock.getElapsedTime().asMilliseconds() > Snake_Base_Speed)
			{
				if (_gamestate != GameStates::Over)
				{
					...
					int over = collision.CheckSnakeError(snake->GetSnake() ,snake->getDirection()  );
					if (over == 1)
					{
						_gamestate = GameStates::Over;
					}
		...
		if (_gamestate == GameStates::Over)
		{
			_data->machine.AddState(StateRef(new Game_Over_State(this->_data, 1, score->getScore())), 1);
